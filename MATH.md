...
$$
\mathcal{T} = \{\text{PREDATION}, \text{MUTUALISM}, \text{PARASITISM}, \text{COMPETITION}, \text{DECOMPOSITION}\}
$$

*(In the codebase implementation, these correspond directly to `PREYS_ON`, `MUTUALISM`, `PARASITISM`, `COMPETITION`, and `DECOMPOSES`).*

For any trophic predation interaction $(u, v, \text{PREDATION}, w)$, node $u$ represents the **predator (consumer)** and node $v$ represents the **prey (resource)**.
...
$$
A_{ij} = \begin{cases} 1 & \text{if } (i, j, \text{PREDATION}) \in \mathcal{E} \lor (j, i, \text{PREDATION}) \in \mathcal{E}, \quad i \neq j \\ 0 & \text{otherwise} \end{cases}
$$
...
$$
\mathcal{R}(i) = 1 + 0.6 \cdot \log_{10}\left(\frac{100}{\max(P_0(i), 1)}\right)
$$

where $P_0(i)$ represents the baseline population/biomass index of species $i$ (standardized to $100$ for baseline reference).

The composite **Keystone Score** $\mathcal{K}(i)$ is computed as:

$$
\mathcal{K}(i) = \left( 0.5 \cdot \frac{C_B(i)}{\max_k C_B(k)} + 0.3 \cdot \frac{C_E(i)}{\max_k C_E(k)} + 0.2 \cdot \frac{C_D(i)}{\max_k C_D(k)} \right) \times \mathcal{R}(i)
$$
...
| Coefficient | Value | Ecological Mechanism |
| :--- | :--- | :--- |
| $\gamma_{\text{loss}}^{BU}$ | $1.00$ | Bottom-up prey loss directly starvation-shocks predator |
| $\gamma_{\text{gain}}^{BU}$ | $0.45$ | Bottom-up prey abundance yields diminishing predator gains |
| $\gamma_{\text{rel}}^{TD}$ | $0.40$ | Top-down predator depletion releases prey from predation |
| $\gamma_{\text{mut}}$ | $0.60$ | Mutualistic interaction multiplier |
| $\gamma_{\text{loss}}^{PH}$ | $0.85$ | Host loss directly starves parasites |
| $\gamma_{\text{rel}}^{HP}$ | $0.15$ | Parasite removal yields modest relief to host |
| $\gamma_{\text{comp}}$ | $0.30$ | Competitive exclusion pressure |
| $\gamma_{\text{sub}}^{DEC}$ | $0.35$ | Decomposer substrate biomass availability |
| $\gamma_{\text{rec}}^{DEC}$ | $0.10$ | Decomposer nutrient recycling to primary producers |
...
\Delta_j \cdot w_e \cdot 1.00 & \text{if } \tau_e = \text{PREDATION}, \text{ consumer } i, \; \Delta_j < 0 \\
\Delta_j \cdot w_e \cdot 0.45 & \text{if } \tau_e = \text{PREDATION}, \text{ consumer } i, \; \Delta_j \ge 0 \\
-\Delta_j \cdot w_e \cdot 0.40 & \text{if } \tau_e = \text{PREDATION}, \text{ resource } i \\
\Delta_j \cdot w_e \cdot 0.60 & \text{if } \tau_e = \text{MUTUALISM} \\
\Delta_j \cdot w_e \cdot 0.85 & \text{if } \tau_e = \text{PARASITISM}, \text{ parasite } i \\
-\Delta_j \cdot w_e \cdot 0.15 & \text{if } \tau_e = \text{PARASITISM}, \text{ host } i \\
-\Delta_j \cdot w_e \cdot 0.30 & \text{if } \tau_e = \text{COMPETITION} \\
\Delta_j \cdot w_e \cdot 0.35 & \text{if } \tau_e = \text{DECOMPOSITION}, \text{ decomposer } i \\
\Delta_j \cdot w_e \cdot 0.10 & \text{if } \tau_e = \text{DECOMPOSITION}, \text{ target } i
\end{cases}
$$
...
$$
x_i = a_x \cdot \left( \frac{M_i}{M_{\text{ref}}} \right)^{-0.25}
$$

where:
* $a_x = 0.314$ (`metabolicScale`).
* $M_{\text{ref}}$ is the median body mass of all basal primary producers in the network.
...
* $e_{ji}$ is the assimilation efficiency ($e_{\text{herb}} = 0.45$ for herbivores consuming plant tissue; $e_{\text{carn}} = 0.85$ for carnivores).
...
$$
\mathcal{A}_j = \frac{B_j}{B_j + h_{\text{symb}}}, \quad h_{\text{symb}} = 0.05
$$

The term $\mathcal{S}_i(\mathbf{B})$ is computed as:

$$
\mathcal{S}_i(\mathbf{B}) = \sum_{j \in \mathcal{N}_{\text{symb}}(i)} \psi_{ij}(B_i, \mathcal{A}_j)
$$

where:
1. **Mutualism:** Depletion of mutualist partner $j$ induces proportional mortality on species $i$:
   $$
   \psi_{ij}^{\text{mut}} = - w_{\text{mut}} \cdot s_{ij} \cdot (1 - \mathcal{A}_j) \cdot B_i
   $$
   ...
   $$
   \psi_{ij}^{\text{sub}} = + x_i \cdot \mu_{\text{sub}} \cdot \mathcal{A}_j \cdot B_i, \quad \mu_{\text{sub}} = 1.40
   $$
2. **Parasite Burden on Host:**
   $$
   \psi_{ij}^{\text{host}} = - \beta_{\text{par}} \cdot s_{ij} \cdot \mathcal{A}_j \cdot B_i, \quad \beta_{\text{par}} = 0.40
   $$
3. **Host Depletion Penalty on Parasite:**
   $$
   \psi_{ij}^{\text{par}} = - s_{ij} \cdot (1 - \mathcal{A}_j) \cdot B_i
   $$
...
1. Compute $\mathbf{B}_{\text{full}}$ over step $h$.
2. Compute $\mathbf{B}_{\text{half}}$ by taking two successive sub-steps of size $\frac{h}{2}$.
3. Estimate the maximum scalar local truncation error:
   $$
   \epsilon = \|\mathbf{B}_{\text{full}} - \mathbf{B}_{\text{half}}\|_\infty = \max_{1 \le i \le n} |B_{\text{full}, i} - B_{\text{half}, i}|
   $$
...
$$
\mathbf{B}_{n+1} = \mathbf{B}_{\text{half}}
$$
...
$$
\mathcal{P}_{\text{stability}} = \frac{|\{i : B_i^* > 10^{-6}\}|}{|\mathcal{V}|}
$$

### 7.2. Dual-Run Differential Trajectory

To isolate the causal effect of an intervention $\mathbf{\Delta}_{\text{input}}$ from ongoing limit cycles or natural oscillations:

1. **Control Run:** $\mathbf{B}_{\text{ctrl}}(t)$ integrated from $\mathbf{B}^*$ over interval $[0, T_{\text{sim}}]$.
2. **Perturbed Run:** $\mathbf{B}_{\text{pert}}(t)$ integrated from $\mathbf{B}_{\text{pert}}(0) = \mathbf{B}^* \odot (1 + \mathbf{\Delta}_{\text{input}} / 100)$.

The reported percentage change over time is:

$$
\Delta \% B_i(t) = \frac{B_i^{\text{pert}}(t) - B_i^{\text{ctrl}}(t)}{B_i^{\text{ctrl}}(t)} \times 100\%
$$
...
$$
TP^{(k+1)}(i) = \min\left( 1 + \sum_{j} \tilde{\omega}_{ij} TP^{(k)}(j), \; 6.0 \right)
$$
...
$$
\mathcal{C} = \frac{|\mathcal{E}_{\text{prey}}|}{|\mathcal{V}|^2} = \frac{L_{\text{trophic}}}{S^2}
$$
...
| **ATN ODE Integration** | Adaptive RK4 Step-Doubling | $\mathcal{O}(N_{\text{steps}} \cdot \|E\|)$ | $\mathcal{O}(\|V\|)$ (SoA vectors) | Brose et al. (2006) |
| **Trophic Position** | Iterative Jacobi/Gauss-Seidel | $\mathcal{O}(M \cdot \|E_{\text{prey}}\|)$ | $\mathcal{O}(\|V\|)$ | Levine (1980) |
